...
def handle_message(message):
    try:
        ...
        # Press the enter key down for 0.5 seconds second, then release it
        if msg == "shoot": 
        ...
        # Move the mouse down by 30 pixels
        if msg == "aim down":
            pydirectinput.moveRel(0, 30, relative=True)
            
        # Move the mouse left by 30 pixels
        if msg == "aim left":
            pydirectinput.moveRel(-30, 0, relative=True)

        # Move the mouse right by 30 pixels
        if msg == "aim right":
            pydirectinput.moveRel(30, 0, relative=True)

        # Move the mouse up by 5 pixels
        if msg == "up":
            pydirectinput.moveRel(0, -5, relative=True)

        # Move the mouse down by 5 pixels
        if msg == "down":
            pydirectinput.moveRel(0, 5, relative=True)
            
        # Move the mouse left by 5 pixels
        if msg == "left":
            pydirectinput.moveRel(-5, 0, relative=True)

        # Move the mouse right by 5 pixels
        if msg == "right":
            pydirectinput.moveRel(5, 0, relative=True)
